...
shm_obs = shared_memory.SharedMemory(create=True, size=int(np.prod(obs_shape) * 1), name="shm_obs")
...
shm_action_left = shared_memory.SharedMemory(create=True, size=3 * 4, name="shm_action_left")
shm_action_right = shared_memory.SharedMemory(create=True, size=3 * 4, name="shm_action_right")
...
action_array_left = np.ndarray(action_shape, dtype=np.int32, buffer=shm_action_left.buf)
action_array_right = np.ndarray(action_shape, dtype=np.int32, buffer=shm_action_right.buf)


# run right player script
subprocess.Popen("python agent_sharedmemory_left.py", shell=True)
subprocess.Popen("python agent_sharedmemory_right.py", shell=True)
time.sleep(1)
...
unity_env = UnityEnvironment(r"C:\Users\junmi\Downloads\PickleBallFinal\PickleBallFinal\Pickleball_Build\dp.exe", side_channels=[string_channel, channel])
...
try: 
    while env.agents:
        ...
        # Read action
        act_left = action_array_left.copy()
        act_right = action_array_right.copy()
        # print(f"[Sender] Action received: {act_right}")

        actions = {env.agents[0]:act_left,env.agents[1]:act_right}
    ...
    for shm in [shm_obs, shm_rew, shm_step, shm_action_left, shm_action_right]:
        try:
            shm.close()
            shm.unlink()
