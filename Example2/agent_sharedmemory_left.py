...
shm_action_left = shared_memory.SharedMemory(name="shm_action_left")
...
try:
    while True:
        ...
        if current_step != last_step:
            ...
            # 
            invert = {0:0, 1:2, 2:1}
            action[1] = invert[action[1]]
            action[2] = invert[action[2]]
            # Send your action back to competition
            action_array_left[:] = action
    ...
    for shm in [shm_obs, shm_rew, shm_step, shm_action_left]:
        try:
            shm.close()
